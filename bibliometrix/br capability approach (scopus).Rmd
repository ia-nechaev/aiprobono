...
>  TITLE ( ( social )  AND  capability )  AND  SUBJAREA ( arts  OR  busi  OR  deci  OR  econ  OR  psyc  OR  soci )  AND  PUBYEAR  >  1980  AND  ( LIMIT-TO ( SRCTYPE ,  "j" ) )  AND  ( LIMIT-TO ( DOCTYPE ,  "ar" ) )  


  
Resulted in 417 documents in SCOPUS database.  I decided to leave only journal articles, as conference papers tend to distort the general picture. Will analyze them separately. Searching only in the title. Otherwise getting more than 9000 results.
...
```{r}
myfile <- 'C:\\Users\\TL66LU\\Nextcloud\\GitHub\\aiprobono\\bibliometrix\\scopus social capabilities.bib'
```
***  
...
sort(Matrix::colSums(A), decreasing = TRUE)[1:100]
```
#### Keyword nw
