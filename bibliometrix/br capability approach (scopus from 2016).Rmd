...
>  TITLE ( capability  AND  approach )  AND  SUBJAREA ( arts  OR  busi  OR  deci  OR  econ  OR  psyc  OR  soci )  AND  PUBYEAR  >  2000  AND  ( LIMIT-TO ( SRCTYPE ,  "j" ) )  AND  ( LIMIT-TO ( DOCTYPE ,  "ar" ) )   


  
Resulted in 782 documents in SCOPUS database.  I decided to leave only journal articles, as conference papers tend to distort the general picture. Will analyze them separately. Searching only in the title. Otherwise getting more than 9000 results.
...
```{r}
myfile <- 'C:\\Users\\TL66LU\\Nextcloud\\GitHub\\aiprobono\\bibliometrix\\scopus_capability_approach(from2016).bib'
```
***  
...
net=networkPlot(NetMatrix,  normalize = "salton", weighted=NULL, n = 20, Title = "Authors' Coupling", type = "fruchterman", size=5,size.cex=T,remove.multiple=TRUE,labelsize=0.8,label.n=10,label.cex=F)
```
